...
def compute_symmetric_states(
    ...
    if obs is not None:
        ...
        # -- left-right
        obs_aug["mapScans"][batch_size:] = _transform_mapScans_obs_left_right(env.unwrapped, obs["mapScans"])
...
def _transform_mapScans_obs_left_right(env: ManagerBasedRLEnv, obs: torch.Tensor) -> torch.Tensor:
    # note: this is hard-coded for grid-pattern of ordering "xy" and size (1.6, 1.0)
    obs[:, :] = obs[:, :].view(-1, 11, 17).flip(dims=[1]).view(-1, 11 * 17)
