...
from rsl_rl.modules import StudentTeacher, StudentTeacherRecurrent, StudentTeacherDepthImage
...
class Distillation:
    ...
    policy: StudentTeacher | StudentTeacherRecurrent | StudentTeacherDepthImage
    ...
    def teacher_act(self, obs: TensorDict) -> torch.Tensor:
        with torch.no_grad():
            # Compute the teacher's deterministic action (drives the environment)
            self.transition.actions = self.policy.evaluate(obs).detach()
            # Teacher action is also the training target
            self.transition.privileged_actions = self.transition.actions
            # Record the student observations
            self.transition.observations = obs
        return self.transition.actions
