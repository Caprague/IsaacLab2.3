...
- 使用 PPO 算法训练 `ActorCriticRMA`（含特权信息编码器 + 历史编码器）
- 不使用深度图，通过本体感知 + 高度扫描点（scan dots, 132维）+ 特权信息训练
...
1. **Rollout 采集**（`num_steps_per_env = update_interval × 24 = 120` 步）：
   ...
   - 深度编码器 `depth_encoder(depth_image, prop)` 产生 `depth_latent`（32维）+ `yaw`（2维）
...
2. **损失计算**（`ppo.py` L325-L336）：
   ...
   - 优化器：`depth_actor_optimizer`（同时优化 depth_actor 和 depth_encoder 参数）

3. **GRU 隐状态分离**：每轮结束后调用 `detach_hidden_states()`

**注意**：代码中还存在 `update_depth_encoder`（纯特征蒸馏，L2损失到 scandots_latent）和 `update_depth_both`（联合优化）两个备选方法，但在 `learn_vision` 中被注释掉，实际只使用 `update_depth_actor`。
...
1. **两阶段蒸馏**：先用 PPO 训练带特权信息的教师策略，再通过动作模仿将深度图感知能力蒸馏到学生策略
2. **特征对齐**：深度编码器输出 32 维，与教师策略的 `scan_encoder` 输出维度一致（`scan_encoder_dims[-1]=32`），学生策略通过 `scandots_latent` 参数注入点无缝替换
3. **时序建模**：GRU（512维隐状态）处理深度图时序依赖，补偿低帧率（每5步更新一次）深度感知
4. **偏航预测**：额外预测 2 维偏航角，弥补深度图无法直接观测全局朝向的缺陷，通过 `delta_yaw_ok` 机制动态控制是否信任预测值
5. **部署友好**：CNN+GRU 结构轻量，输入仅深度图+本体感知（无特权信息），适合实机部署
